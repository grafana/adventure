...
def _action_display(action_key):
    """Return translated label for an action key. Literals below ensure xgettext extracts them."""
    _labels = {
        "accept his offer": _("accept his offer"),
        "accept quest": _("accept quest"),
        "blacksmith": _("blacksmith"),
        "chapel": _("chapel"),
        "check sword": _("check sword"),
        "cheat": _("cheat"),
        "cool forge": _("cool forge"),
        "decline his offer": _("decline his offer"),
        "exit the cave": _("exit the cave"),
        "go back": _("go back"),
        "go east": _("go east"),
        "go to forest": _("go to forest"),
        "go to town": _("go to town"),
        "go towards light": _("go towards light"),
        "heat forge": _("heat forge"),
        "kill him": _("kill him"),
        "look around": _("look around"),
        "look at sword": _("look at sword"),
        "mysterious man": _("mysterious man"),
        "pray": _("pray"),
        "quest giver": _("quest giver"),
        "rebuild blacksmith": _("rebuild blacksmith"),
        "request sword": _("request sword"),
        "take the box": _("take the box"),
        "wizard": _("wizard"),
    }
    return _labels.get(action_key, action_key)
...
class AdventureGame:
    def __init__(self):
        ...
        self.adventurer_name = input(_("Enter your name, brave adventurer: "))
        ...
        self.locations = {
            "start": {
                "description": _("You are at the beginning of your adventure. There's a path leading north towards a town, and another path leading east towards a forest."),
                "actions": {
                    "go to town": {"next_location": "town"},
                    "go to forest": {"next_location": "forest"},
                    "cheat": {"message": _("You cheat and get a sword. You feel guilty"), "effect": self.cheat}
                }
            },
            "forest": {
                "description": _("You are in a dark forest. The trees are tall and the air is thick, you can make out a faint trail heading further east."),
                "actions": {
            ...
            "cave": {
                "description": _("You enter a dark cave at the end of the trail. The air is cold and damp. You see a faint light at the end of the cave."),
                "actions": {
            ...
            "treasure": {
                "description": _("You find a treasure chest at the end of the cave. Inside is a small decorative wooden box with no visible way of opening it."),
                "actions": {
                    "take the box": {"message": _("You take the box and place it in your pocket."), "effect": self.take_box, "pre_requisite": self.box_still_in_chest},
                    "exit the cave": {"message": _("You retrace your steps and go back to where you first started your adventure"), "next_location": "start"}
                }
            },
            "blacksmith": {
                "description": _("You are at the blacksmith's forge. The blacksmith is busy working."),
                "actions": {
                    "request sword": {
                        "message": _("You ask the blacksmith to forge you a new sword."),
                        "effect": self.request_sword,
                        "pre_requisite": self.is_blacksmith_alive
                    },
                    "cool forge": {
                        "message": _("You pour water on the forge. The coals sizzle."),
                        "effect": self.cool_forge,
                        "pre_requisite": self.is_forge_heating
                    },
                    "heat forge": {
                        "message": _("You add more coal to the forge, increasing its heat."),
                        "effect": self.heat_forge,
                        "pre_requisite": self.is_sword_requested
                    },
                    "check sword": {
                        "message": _("You check if the sword is ready."),
            ...
            "town": {
                "description": _("You are in a bustling town. People are going about their business. You see a blacksmith, a mysterious man wandering the streets, a quest giver, and a chapel."),
                "actions": {
                    "blacksmith": {"next_location": "blacksmith", "pre_requisite": self.is_blacksmith_alive, "effect": self.enter_blacksmith},
                    "rebuild blacksmith": {"message": _("You help the town rebuild the blacksmith."), "effect": self.rebuild_blacksmith, "pre_requisite": self.is_blacksmith_dead},
            ...
            "mysterious man": {
                "description": _("You meet a mysterious man. He offers to enhance your sword with magic."),
                "actions": {
                    "accept his offer": {"message": _("A great choice indeed. Your sword is now enchanted with great power."), "effect": self.evil_wizard},
                    "decline his offer": {"message": _("You will not get another chance. ACCEPT MY OFFER!")},
                    "go to town": {"next_location": "town"}
                }
            },
            "wizard": {
                "description": _("You meet a wizard. He yells 'Are you here to kill me?!'"),
                "actions": {
                    "kill him": {"message": _("You attempt to kill the wizard."), "pre_requisite": self.is_quest_accepted, "effect": self.kill_wizard},
                    "go to town": {"next_location": "town"}
                }
            },
            "quest": {
                "description": _("You meet a quest giver. He offers you a quest to defeat the evil wizard."),
                "actions": {
                    "accept quest": {"message": _("You tell the quest giver you would like to accept..."), "effect": self.quest_giver},
                    "go to town": {"next_location": "town"}
                }
            },
            "chapel": {
                "description": _("You enter the chapel. The priest greets you warmly."),
                "actions": {
                    "look at sword": {"message": _("The priest looks at your sword"), "effect": self.priest},
                    "pray": {"message": _("You pray for guidance.")},
                    "go to town": {"next_location": "town"}
                }
            }
        }
    ...
    def take_box(self):
        if self.has_box:
            return _("You already have the box.")
        
        self.has_box = True
        return _("You hear a slight hum coming from the box as you touch it.")
    ...
    def enter_blacksmith(self):
        if self.has_box:
            logging.info(_("As you enter the blacksmith you trip over a small stone slab. You feel lighter somehow."))
    ...
    def rebuild_blacksmith(self):
        if self.has_box:
            logging.info(_("While rebuilding the blacksmith, amongst the ashes you find the burnt remains of the decorative box. Laying beside it is a glowing, unburnt, piece of parchment which reads: 'Congratulations, Adventurer!'. Below it is a long cryptic looking message."))
        ...
        self.blacksmith_burned_down = False
        self.cool_forge()
        return _("You help the town rebuild the blacksmith. The blacksmith is grateful.")
    ...
    def cool_forge(self):
        self.heat = 0
        self.is_heating_forge = False
        return _("You throw a bucket of water over the forge. The coals sizzle and the forge cools down completely.")

    def heat_forge(self):
        self.is_heating_forge = True
        return _("You fire up the forge and it begins heating up. You should wait a while before checking on the sword.")

    def request_sword(self):
        if self.has_sword:
            return _("You already have a sword. You don't need another one.")
        
        if self.failed_sword_attempts > 0 and self.failed_sword_attempts < 3:
            self.sword_requested = True
            if self.is_heating_forge:
                logging.warning(_("You requested another sword, but the forge is still hot!"))
            return _("The blacksmith looks at you with disappointment. He says, 'Fine, but be more careful this time! If the forge gets too hot, the sword will melt.'")
        elif self.failed_sword_attempts >= 3:
            logging.error(_("The blacksmith refuses to forge you another sword. You have wasted too much of his time."))
            return _("The blacksmith refuses to forge you another sword. You have wasted too much of his time.")
        
        self.sword_requested = True
        return _("The blacksmith agrees to forge you a sword. It will take some time and the forge needs to be heated to the correct temperature however.")
    ...
    def cheat(self):
        ...
        return _("You should continue north you cheater.")
    ...
    def kill_wizard(self):
        if self.has_holy_sword:
            ...
            return _("You strike the wizard down with your holy sword. The town cheers for you. Your adventure has come to an end.")

        if self.has_evil_sword:
            self.current_location = "town"
            self.game_active = False  # End the game if the attempt fails fatally
            logging.critical(_("Your sword falters as you try to strike the wizard down. The wizard laughs as you fall to the ground."))
            return _("The wizard laughs as you strike him down. The sword was cursed. You have failed. The adventure ends here.")

        if self.has_sword:
            self.current_location = "town"
            logging.warning(_("Your sword is not powerful enough to defeat the wizard. Your sword shatters, you should probably get a new one."))
            self.has_sword = False
            return _("You try to strike the wizard down but your sword is not powerful enough.")


    def priest(self):
        if self.has_holy_sword:
            return _("I have already blessed your sword child, go now and use it well.")
        ...
        if self.has_sword and not self.has_evil_sword:
            ...
            return _("The priest blesses your sword. You feel a warm glow.")
        ...
        if self.has_evil_sword:
            ...
            logging.warning(_("The priest transfers the curse from the sword to himself. He falls to the ground."))
            logging.warning(_("The sword is now blessed. You feel a warm glow."))
            return _("The priest looks at your sword with fear. My child, this sword is cursed. I will transfer the curse to me.")
        else:
            return _("The priest looks at your empty hands. You feel a little embarrassed.")
    ...
    def check_sword(self):
        ...
        if self.heat >= 10 and self.heat <= 20:
            ...
            current_span.add_event(_("Sword forged"))
            return _("The sword is ready. You take it from the blacksmith.")
        elif self.heat >= 21:
            self.sword_requested = False
            self.failed_sword_attempts += 1
            current_span.add_event(_("The sword has completely melted!"))
            return _("The sword has completely melted! The blacksmith looks at you with disappointment.")
        else:
            current_span.add_event(_("To cold!"))
            return _("The forge is not hot enough yet. The blacksmith tells you to wait.")
    ...
    def evil_wizard(self):
        ...
        logging.error(_("The evil wizard has enchanted your sword with dark magic. You feel a chill run down your spine. This is a warning..."))
        return _("You feel funny but powerful. Maybe I should accept a quest.")
    
    def quest_giver(self):
        current_span = trace.get_current_span()
        if self.has_evil_sword:
            current_span.add_event(_("You killed the quest giver with your evil sword!"))
            logging.critical(_("The sword whispers; I killed them! you will never destroy the wizard with me in your hands! Hahahaha"))
            self.current_location = "town"
            return _("The quest giver turns pale. They collapse. Dead! What do I do now?")
        elif self.has_holy_sword:
            logging.warning(_("The sword whispers; I will help you defeat the wizard. I am your only hope."))
            self.quest_accepted = True
            return _("Wow! You have such a powerful sword. I will give you a quest to defeat the evil wizard.")
        elif self.has_sword:
            self.quest_accepted = True
            current_span.add_event(_("He's not really impressed with your sword."))
            logging.warning(_("Ok, if you're sure... But it seems your sword may not be powerful enough to defeat the wizard."))
            return _("The quest giver tentatively gives you a quest to defeat the evil wizard.")
        else:
            return _("You don't have a sword. The quest giver looks at you with disappointment.")
    ...
    def list_actions(self):
        ...
        # Create numbered action list (translated labels)
        numbered_actions = [f"{Colors.MAGENTA}{i+1}. {_action_display(action)}{Colors.RESET}" for i, action in enumerate(actions)]
        return _("Available actions: %s") % ", ".join(numbered_actions)

    def process_command(self, command):
        if command.lower() in ["quit", "exit"]:
            self.game_active = False
            return _("You have ended your adventure.")
        ...
        if command.lower() in actions:
            ...
            if "pre_requisite" in action and not action["pre_requisite"]():
                return _("You can't do that right now.")
            ...
            return _("I don't understand that command.")
    ...
    def play(self):
        ...
        print(_("Welcome to your text adventure! Type 'quit' to exit."))
        logging.info(_("Welcome to your text adventure! Type 'quit' to exit."))
        ...
        restart_command = input(_("Would you like to restart the adventure? (yes/no): ")).strip().lower()
    ...
    def restart_adventure(self):
        ...
        new_name = input(_("Enter your name if you'd like to change it, or press Enter to keep the same name: ")).strip()
        if new_name:
